// ====== Config ======
const BASE_URL = "http://localhost:8085/api/transactions";
const ACCOUNTS_API_BASE = "http://localhost:8081/api/accounts/customer";
...
function buildSearchUrl() {
	const params = new URLSearchParams();
	 params.set("accountNumber", state.accountNumber || "");
	 params.set("type", state.type || "ALL");
	 if (state.from) params.set("from", state.from);
	 if (state.to) params.set("to", state.to);
	 params.set("sortBy", state.sortBy || "transactionDate");
	 params.set("dir", state.direction || "DESC"); // 'dir' instead of 'direction' to match backend
  ...
  const rows = items.map(it => {
    ...
    return `
      <tr>
        ...
        <td>${it.status ?? ""}</td>
        <td>${it.remarks ?? ""}</td>
		<td class="fw-semibold">${formatAmount(it.balanceAfterTxn)}</td>
      </tr>
    `;
  }).join("");
...
async function fetchAccountNumber() {
  ...
  try {
    const res = await fetch(`${ACCOUNTS_API_BASE}/${encodeURIComponent(customerId)}`, {
    ...
    // Use the correct field; fallback if backend ever changes field name:
    const account = accounts[0];
    state.accountNumber = account.accountNo || account.accountNumber;
    if (els.accountLabel) {
      els.accountLabel.textContent = `Account No: ${state.accountNumber}` +
...
els.form.addEventListener("submit", (ev) => {
  ...
  if (!state.accountNumber) {
    showError("Account not available.");
    return;
  }
...
els.latestBtn.addEventListener("click", async () => {
  ...
  showLoading(true);
  try {
    // You may need to adjust this endpoint if your backend expects with-balance
    // "/with-balance/{accountNumber}/{count}"
    const url = `${BASE_URL}/with-balance/${encodeURIComponent(state.accountNumber)}/${state.latestN}`;
...
// ====== Fetch and display paged results ======
...
window.addEventListener("DOMContentLoaded", async () => {
  const account = await fetchAccountNumber();
  if (account) fetchAndRender();
});
